...
class Account_facebook_model extends CI_Model {
	...
	{
		return $this->db->get_where('apninv_a3m_account_facebook', array('account_id' => $account_id))->result();
	}
	...
	{
		return $this->db->get_where('apninv_a3m_account_facebook', array('facebook_id' => $facebook_id))->row();
	}
	...
	function insert($account_id, $facebook_id)
	{
		...
		{
			$this->db->insert('apninv_a3m_account_facebook', array('account_id' => $account_id, 'facebook_id' => $facebook_id, 'linkedon' => mdate('%Y-%m-%d %H:%i:%s', now())));
			return TRUE;
		}
		return FALSE;
	}
	...
	{
		$this->db->delete('apninv_a3m_account_facebook', array('facebook_id' => $facebook_id));
	}
