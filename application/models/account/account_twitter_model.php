...
class Account_twitter_model extends CI_Model {
	...
	{
		return $this->db->get_where('apninv_a3m_account_twitter', array('account_id' => $account_id))->result();
	}
	...
	{
		return $this->db->get_where('apninv_a3m_account_twitter', array('twitter_id' => $twitter_id))->row();
	}
	...
	function insert($account_id, $twitter_id, $oauth_token, $oauth_token_secret)
	{
		...
		{
			$this->db->insert('apninv_a3m_account_twitter', array('account_id' => $account_id, 'twitter_id' => $twitter_id, 'oauth_token' => $oauth_token, 'oauth_token_secret' => $oauth_token_secret, 'linkedon' => mdate('%Y-%m-%d %H:%i:%s', now())));
			return TRUE;
		}
		return FALSE;
	}
	...
	{
		$this->db->delete('apninv_a3m_account_twitter', array('twitter_id' => $twitter_id));
	}
