...
class Rel_account_role_model extends CI_Model {
  ...
  function get()
  {
    return $this->db->get('apninv_a3m_rel_account_role')->result();
  }
  ...
  function get_by_account_id($account_id)
  {
    ...
    $this->db->join('apninv_a3m_acl_role', 'apninv_a3m_rel_account_role_model.role_id = apninv_a3m_acl_role.id');
    $this->db->where("apninv_a3m_rel_account_role_model.account_id = $account_id AND apninv_a3m_acl_role.suspendedon IS NULL");
  ...
  function update($account_id, $role_id)
  {
    ...
    {
      $this->db->insert('apninv_a3m_rel_account_role', array('account_id' => $account_id, 'role_id' => $role_id));
    }
  }
  ...
  function update_batch($account_id, $role_ids)
  {
    ...
    if( count($role_ids) > 0)
    {
      ...
      $this->db->insert_batch('apninv_a3m_rel_account_role', $batch);
    }
  }
  ...
  {
    $this->db->delete('apninv_a3m_rel_account_role', array('account_id' => $account_id, 'role_id' => $role_id));
  }
  ...
  {
    $this->db->delete('apninv_a3m_rel_account_role', array('account_id' => $account_id));
  }
  ...
  {
    $this->db->delete('apninv_a3m_rel_account_role', array('role_id' => $role_id));
  }
}