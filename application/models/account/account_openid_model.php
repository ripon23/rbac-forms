...
class Account_openid_model extends CI_Model {
	...
	{
		return $this->db->get_where('apninv_a3m_account_openid', array('account_id' => $account_id))->result();
	}
	...
	{
		return $this->db->get_where('apninv_a3m_account_openid', array('openid' => $openid))->row();
	}
	...
	function insert($openid, $account_id)
	{
		...
		{
			$this->db->insert('apninv_a3m_account_openid', array('openid' => $openid, 'account_id' => $account_id, 'linkedon' => mdate('%Y-%m-%d %H:%i:%s', now())));
			return TRUE;
		}
		return FALSE;
	}
	...
	{
		$this->db->delete('apninv_a3m_account_openid', array('openid' => $openid));
	}
