...
class Ref_location_model extends CI_Model {
 ...
 WHERE (location_bbs2011.loc_type = "DV") ORDER BY location_bbs2011.loc_name_en ASC');					
	...
	function get_district_name_from_id($district_id)
	{
	$this->db->where('district',$district_id);
	$query = $this->db->get('location_bbs2011');
	return $query->row()->loc_name_en;	
	}
	
	function get_all_district_info()
	{
	$this->db->order_by('loc_name_en','ASC');
	$this->db->where('loc_type','DT');
	$query = $this->db->get('location_bbs2011');	
	return $query->result();	
	}
